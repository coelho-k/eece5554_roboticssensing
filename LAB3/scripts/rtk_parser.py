...
from rtk_gps.msg import rtk_data
...
def get_frame_id():
        ...
        """
        frame_id = rospy.get_param('~frame_id', 'rtk_gps')
        # Add the TF prefix
        prefix = ""
...
def main():
    ...
    pub = rospy.Publisher('rtk_data', rtk_data)
    # pub_raw = rospy.Publisher('raw_data', String)
    rate = rospy.Rate(1)

    serial_port = rospy.get_param('~port', '/dev/ttyACM0')
    serial_baud = rospy.get_param('~baud', 460800)
